...
class IntegrationTester {
  ...
  async testDynamoDBIntegration(): Promise<TestResult[]> {
    ...
    // Test 1: Health check
    tests.push(
      await this.runTest('DynamoDB Health Check', async () => {
    ...
    tests.push(
      await this.runTest('DynamoDB Get All Tracks', async () => {
    ...
    tests.push(
      await this.runTest('DynamoDB Get Track By ID', async () => {
        const tracks = await dynamoDBService.getAllTracks();
        if (tracks.length > 0) {
          const firstTrack = tracks[0];
          const retrievedTrack = await dynamoDBService.getTrackById(
            firstTrack.id
          );
          if (!retrievedTrack) {
            throw new Error(
              `Could not retrieve track with ID: ${firstTrack.id}`
            );
          }
          ...
          console.log(
            'No tracks available for ID test - this is expected for empty database'
          );
        }
      })
    );
    ...
    tests.push(
      await this.runTest('DynamoDB Non-existent Track Handling', async () => {
        const nonExistentTrack =
          await dynamoDBService.getTrackById('non-existent-id');
        if (nonExistentTrack !== null) {
          throw new Error(
            'Expected null for non-existent track, but got a result'
          );
        }
      })
    );
  ...
  async testS3Integration(): Promise<TestResult[]> {
    ...
    tests.push(
      await this.runTest('S3 Secure URL Generation', async () => {
    ...
    tests.push(
      await this.runTest('S3 File Existence Check', async () => {
    ...
    tests.push(
      await this.runTest('S3 File Metadata Retrieval', async () => {
        const testKey = 'non-existent-file.mp3';
        ...
        if (metadata !== null) {
          console.log(
            `Unexpected metadata for non-existent file: ${JSON.stringify(metadata)}`
          );
        }
      })
    );
    ...
    tests.push(
      await this.runTest('S3 Audio URLs with Fallbacks', async () => {
        ...
        try {
          ...
          if (
            ...
            console.log(
              'No audio files found for fallback test - this is expected for empty bucket'
            );
          } else {
            throw error;
          }
  ...
  async testStreamingPlatformIntegration(): Promise<TestResult[]> {
    ...
    tests.push(
      await this.runTest(
        'Streaming Platforms - Get Active Platforms',
        async () => {
          const activePlatforms =
            streamingPlatformsService.getActivePlatforms();
    ...
    tests.push(
      await this.runTest('Streaming Platforms - URL Validation', async () => {
        const testCases = [
          {
          ...
          {
            platform: 'spotify' as const,
            url: 'https://invalid-domain.com/track/test',
            expected: false,
          },
          {
    ...
    tests.push(
      await this.runTest(
        'Streaming Platforms - Search URL Generation',
        async () => {
          ...
          for (const platform of platforms) {
            ...
            try {
              ...
              throw new Error(
                `Invalid search URL generated for ${platform}: ${searchUrl}`
              );
            }
          }
    ...
    tests.push(
      await this.runTest(
        'Streaming Platforms - Genre Suggestions',
        async () => {
          const testGenres = ['electronic', 'ambient', 'rock', 'unknown-genre'];

          for (const genre of testGenres) {
            const suggestions =
              streamingPlatformsService.getSuggestedPlatforms(genre);

            if (!Array.isArray(suggestions)) {
              throw new Error(
                `getSuggestedPlatforms returned non-array for genre: ${genre}`
              );
            }
          ...
          console.log(
            `Generated platform suggestions for ${testGenres.length} genres`
          );
        }
      )
    );
  ...
  async testErrorHandling(): Promise<TestResult[]> {
      ...
      await this.runTest('Error Handling - Network Errors', async () => {
        ...
        try {
          ...
          if (!(error instanceof Error)) {
            throw new Error(
              'Error handling did not return proper Error object'
            );
          }
        }
      })
    ...
    tests.push(
      await this.runTest('Error Handling - Invalid URLs', async () => {
        const invalidUrls = [
          'not-a-url',
          'http://',
        ...
        for (const url of invalidUrls) {
          ...
          if (isValid) {
            throw new Error(
              `Invalid URL was incorrectly validated as valid: ${url}`
            );
          }
        }
